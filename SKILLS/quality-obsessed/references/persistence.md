...
The 30-loop floor applies while meaningful in-scope risks remain. It is a persistence guarantee, not permission to fabricate work. A small or already-saturated artifact may stop earlier only with captured evidence that another loop would add no user value, risk reduction, craft, maintainability, or proof.
...
If that capability is unavailable, keep the same compact checkpoint in the current task and disclose the limitation. Never claim background persistence. This record requirement is independent from durable-goal activation and does not by itself turn an ordinary multi-slice change into a 30-loop mission.

## Dynamic backlog

Seed only findings supported by the baseline, applicable gates, executed output, source inspection, recovery autopsy, or independent review. Do not pre-create 30 weaknesses. Replenish the backlog as new evidence appears inside the frozen mission scope.
...
Use one fresh reviewer before Loop 10 when the surface is substantial and independent evidence can change the backlog. At Loop 30, inspect the artifact and issue exactly one continuation verdict:
...
Continue beyond Loop 30 without a hard maximum while the verdict remains `continue`.
